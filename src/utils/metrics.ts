...
export const fetchVastAIMetrics = async (): Promise<IHostPerformance[]> => {
  let instances: IHostPerformance[] = [];
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      logger.error(
        `FETCH METRICS FROM VASTAI FAILED ${(e as Error).message.toUpperCase().slice(0, 60)}`,
      );
      return reject(e);
    }
  });
...
export const fetchTensordockMetrics = async (): Promise<IHostPerformance[]> => {
  ...
  const headers = {
    ...
    Cookie:
      "PHPSESSID=f28kd9e55ih3m5l9m39ddfq0pd; _fw_crm_v=b7c41f54-b64e-4a4d-90e6-664b0b4f623b",
    "Sec-Ch-Ua":
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      logger.error(
        `FETCH METRICS FROM TENSORDOCK FAILED ${(e as Error).message.toUpperCase().slice(0, 60)}`,
      );
      return reject(e);
    }
  });
...
export const fetchPaperspaceMetrics = async (): Promise<IHostPerformance[]> => {
  return new Promise(async (resolve, reject) => {
    try {
