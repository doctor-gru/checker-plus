...
const markup = (host: IHost) => {
  const isH100orH200 = host.specs.gpu.some(
    (gpu) => gpu.type.includes("H100") || gpu.type.includes("H200"),
  );
  ...
  return {
    ...
    specs: {
      ...
      gpu: host.specs.gpu.map((gpu) => ({
        ...gpu,
        price: String(isH100orH200 ? Number(gpu.price) * 0.8 : Number(gpu.price) * 0.5),
      })),
    },
  };
};

export const updateAvailableInstances =
    ...
    try {
      ...
      if ($.success == true) {
        ...
        oldHosts.forEach((oldHost) => {
          ...
          if (index == -1) removingList.push(oldHost._id);
          else allHosts.splice(index, 1);
        });
